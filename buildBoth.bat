...
REM   1) ae2vm-<ver>_forge_1.10.2.jar           针对检测版（blockedMode=crash，检测到该作者 mod 游戏闪退）
REM   2) ae2vm-nodetect-<ver>_forge_1.10.2.jar  无针对检测版（blockedMode=warn，只警告不闪退）
REM 注意：故意闪退的是【游戏运行时】（throw RuntimeException），
REM       编译过程本身永不失败。两个 jar 共用 modId=ae2vm，
REM       启动游戏前请删掉不用那个，否则会 duplicate modId 报错。
REM 目标 mods 文件夹由 gradle.properties 的 mods_folder 指定（默认 1.10.2）。
REM 构建需要 JDK 8（gradle.properties 的 org.gradle.java.home 已指向）。
REM ============================================================
...
echo   - ae2vm-<version>_forge_1.10.2.jar            （针对检测，会闪退）
echo   - ae2vm-nodetect-<version>_forge_1.10.2.jar   （无针对检测，只警告）
echo  启动游戏前只保留其中一个！
echo ============================================================
goto :eof
