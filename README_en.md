...
1. Install [NeoForge 26.1.2](https://neoforged.net/) and [Applied Energistics 2 26.1.10-beta+](https://www.curseforge.com/minecraft/mc-mods/applied-energistics-2).
2. Put `ae2vm-1.10.7_neoforge_26.1.2.jar` into `mods/` (remove old versions).
   - The no-detection build is `ae2vm-nodetect-1.10.7_neoforge_26.1.2.jar` (warns instead of crashing on incompatible author mods).
...
│   └── AE2VMConfig.java             # config entry (ModConfigSpec → config/ae2vm-common.toml, proxyEnabled toggle)
├── mixin/
...
| Mod ID | `ae2vm` |
| Name | AE2 VM |
| Version | 1.10.7 (jar suffix `_neoforge_26.1.2`) |
...
```bash
set JAVA_HOME=D:\Java26   # requires Java 26 / 25 (Gradle 9.7.0)
...
# Output (<ver> = current version, e.g. 1.10.7; with _neoforge_26.1.2 suffix)
# build/libs/ae2vm-<ver>_neoforge_26.1.2.jar            (crashes on incompatible author mods)
# build/libs/ae2vm-nodetect-<ver>_neoforge_26.1.2.jar   (only warns)
```
