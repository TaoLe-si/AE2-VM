...
/**
 * AE2VM 配置（NeoForge {@link ModConfigSpec} → {@code config/ae2vm-common.toml}）。
 *
 * <p>配置迁移说明：旧版使用 Cloth Config API（AutoConfig + Gson）读写
 * {@code config/ae2vm.json}；现改为 NeoForge 原生 TOML 配置：
 * <ul>
 *   <li>配置文件：{@code config/ae2vm-common.toml}（COMMON 分类，客户端/服务端共用）；</li>
 *   <li>Configured（NeoForge 版）会自动扫描本 mod 注册的 {@code ModConfig}，在游戏内
...
public final class AE2VMConfig {
    ...
    /** 已构建的 COMMON 配置规格，注册于 mod 构造函数（{@code ModContainer.registerConfig}）。 */
    public static final ModConfigSpec COMMON_SPEC;

    /** 是否启用 AE2-VM 代理（拦截 AE2 合成计算，用 VM 引擎替代递归计算）。默认 true。 */
    public static final ModConfigSpec.BooleanValue PROXY_ENABLED;

    /** 是否输出调试日志（被 {@code if (isDebugLogging())} 包裹的额外 LOGGER.info 调用）。默认 false。 */
