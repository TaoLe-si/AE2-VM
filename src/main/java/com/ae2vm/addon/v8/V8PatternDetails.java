...
public final class V8PatternDetails implements IPatternDetails {
    ...
    public V8PatternDetails(ICraftingPatternDetails delegate) {
        ...
        for (int i = 0; i < rawInputs.size(); i++) {
            this.inputs[i] = new V8Input(this.delegate, rawInputs.get(i));
        }
    ...
    private static final class V8Input implements IInput {
        ...
        public IAEStack[] getPossibleInputs() {
            // AE2 8.0.0 没有多候选输入可枚举：ICraftingPatternDetails 上不存在
            // getSubstituteInputs(int)（8.2.0 才加），且 appeng.helpers.CraftingPatternDetails
            // 的字节码里对 net.minecraft.item.crafting.Ingredient 的引用数是 0（8.4.7 是 10）
            // ——即 1.16.1 的样板根本没有标签/多选输入这个概念。
            // AE2 自家 CraftingTreeNode.request() 在 canSubstitute() 分支也只对 this.what 做
            // findFuzzy(what, IGNORE_ALL)，候选键就是 template 这一个；模糊维度由下游
            // KeyCounter.findFuzzy 负责，与本 fork 在 1.16.4/1.16.5 上的行为一致。
