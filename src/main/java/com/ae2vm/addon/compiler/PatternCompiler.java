...
public class PatternCompiler {
   ...
   private static AEKey containerItem(IInput input, AEKey template) {
      ...
      appeng.api.storage.data.IAEStack result =
            input.getContainerItem(template instanceof AEItemKey ik ? ik.toStack(1) : null);
   ...
   private static boolean isProcessingPattern(IPatternDetails pattern) {
      ...
      // AE2 1.18.1 (forge/v10.x): IMolecularAssemblerSupportedPattern 不存在
      // 改用 appeng.crafting.pattern.AECraftingPattern（crafting 模式 vs processing 模式分流）
      // — 同样语义：crafting patterns 跑在分子装配机；processing patterns 走外部机器配方
      return !(pattern instanceof appeng.crafting.pattern.AECraftingPattern);
   }
   ...
   public static IPatternDetails findCompiledByOutput(AEKey outputKey) {
      ...
      if (outputKey != null) {
         net.minecraft.world.item.Item outputItem = (outputKey instanceof appeng.api.stacks.AEItemKey k) ? k.getItem() : null;
         ...
         for (Entry<IPatternDetails, CraftingBytecode> entry : COMPILED_PATTERNS.entrySet()) {
            ...
            if (patternOutput != null && patternOutput.what() != null) {
               if (outputItem != null && patternOutput.what() instanceof appeng.api.stacks.AEItemKey pItem
                       && pItem.getItem() == outputItem) {
                  return entry.getKey();
               }
            ...
            for (GenericStack out : outputs(entry.getKey())) {
               if (out != null && out.what() != null) {
                  if (outputItem != null && out.what() instanceof appeng.api.stacks.AEItemKey oItem
                          && oItem.getItem() == outputItem) {
                     return entry.getKey();
                  }
