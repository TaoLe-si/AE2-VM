   ...
   private static CraftingBytecode compilePattern(IPatternDetails pattern) {
      ...
      for (IInput inputEntry : patternInputs) {
         ...
         if (possibleInputs == null || possibleInputs.length == 0) {
            ...
            // 1.19+ 的 getRemainingKey 等同于 1.18.1 的 getContainerItem(template) — 返回 crafting 模式 Recipe.getRemainingItems 容器 key
            // processing 模式输入 getContainerItem 返回 null（同样语义）
            AEKey remainingKey = inputEntry.getContainerItem(inputKey);
