...
public class PatternCompiler {
   ...
   // --- AE2 v9 (1.17.1) IAEStack → shim GenericStack 转换（v10+ 直接返回 GenericStack） ---

   /** v9 {@code getPrimaryOutput()} 返回 IAEStack；转 shim GenericStack（null 安全）。 */
   private static GenericStack primaryOutput(IPatternDetails pattern) {
      return GenericStack.wrap((IAEItemStack) pattern.getPrimaryOutput());
   }

   /** v9 {@code getOutputs()} 返回 IAEStack[]；转 shim GenericStack[]（null 安全）。 */
   private static GenericStack[] outputs(IPatternDetails pattern) {
      appeng.api.storage.data.IAEStack[] raw = pattern.getOutputs();
      if (raw == null) return new GenericStack[0];
      GenericStack[] out = new GenericStack[raw.length];
      for (int i = 0; i < raw.length; i++) {
         out[i] = GenericStack.wrap((IAEItemStack) raw[i]);
      }
      return out;
   }

   /** v9 {@code getPossibleInputs()} 返回 IAEStack[]；转 shim GenericStack[]（null 安全）。 */
   private static GenericStack[] possibleInputs(IInput input) {
      appeng.api.storage.data.IAEStack[] raw = input.getPossibleInputs();
      if (raw == null) return new GenericStack[0];
      GenericStack[] out = new GenericStack[raw.length];
      for (int i = 0; i < raw.length; i++) {
         out[i] = GenericStack.wrap((IAEItemStack) raw[i]);
   ...
   /** v9 {@code getContainerItem(AEKey)}：把 shim key 转 IAEStack 再调 v9 API，包装回来。 */
   private static AEKey containerItem(IInput input, AEKey template) {
      if (template == null) return null;
      appeng.api.storage.data.IAEStack result =
            input.getContainerItem((template instanceof AEItemKey) ? ((AEItemKey) template).toStack(1) : null);
      if (result == null) return null;
      return AEItemKey.wrap((IAEItemStack) result);
   }
   ...
   private static boolean isProcessingPattern(IPatternDetails pattern) {
      ...
      // AE2 v8 (1.16.5): 没有 v10+ 的 appeng.crafting.pattern.* 分流，v8 用统一的
      // ICraftingPatternDetails 承载 `isCraftable()` 标志 —— false 即为 processing 模式
      // （走外部机器配方），true 为分子装配机的 crafting 模式。语义与 v10+ 一致。
      appeng.api.networking.crafting.ICraftingPatternDetails v8 =
            com.ae2vm.addon.v8.V8PatternDetails.unwrap(pattern);
      if (v8 != null) {
         return !v8.isCraftable();
      }
      return true; // 非常规实现：按 processing 处理（保守，走外部机器）
   }
   ...
   public static void registerFuzzyGroups(IPatternDetails pattern) {
      ...
      // (v8) appeng.helpers.CraftingPatternDetails 是 AE2 v8 唯一的原生 ICraftingPatternDetails；
      // uel(1.12.2) 里同类改名成 appeng.helpers.PatternHelper（javap 全 jar 扫 implements 只有它）
      // 实现，crafting/processing 由 isCraftable() 区分 —— 等价于 v9 的 AEProcessingPattern。
      appeng.api.networking.crafting.ICraftingPatternDetails v8p =
            com.ae2vm.addon.v8.V8PatternDetails.unwrap(pattern);
      boolean nativeAE2Processing = v8p != null
            && v8p instanceof appeng.helpers.PatternHelper;
      ...
      for (IInput inputEntry : patternInputs) {
         GenericStack[] possibleInputs = possibleInputs(inputEntry);
   ...
   public static java.util.Set<AEKey> getFuzzyGroup(AEKey key) {
      ...
      return group != null ? group : java.util.Collections.singleton(key);
   }
   ...
   private static IPatternDetails unwrapScaled(IPatternDetails pattern) {
      ...
      while (isScaledPattern(current)) {
         try {
            java.lang.reflect.Method method = current.getClass().getMethod("getOriginal");
   ...
   private static boolean hasUsableOutput(IPatternDetails pattern) {
      try {
         GenericStack[] outputs = outputs(pattern);
   ...
   private static CraftingBytecode compilePattern(IPatternDetails pattern) {
      ...
      CraftingBytecode.Builder builder = new CraftingBytecode.Builder();
      GenericStack primaryOutput = primaryOutput(pattern);
      ...
      for (IInput inputEntry : patternInputs) {
         ...
         if (possibleInputs == null || possibleInputs.length == 0) {
            ...
            // AE2 v9 (1.17.1): IInput.getContainerItem(IAEStack) — 与 v10 同名同语义，
            // 走 containerItem() helper 做 shim key ↔ IAEStack 转换
            AEKey remainingKey = containerItem(inputEntry, inputKey);
            if (remainingKey != null && remainingKey.equals(inputKey)) {
               ...
               if (inputEntry instanceof IFiniteUseInput) {
                   IFiniteUseInput f = (IFiniteUseInput) inputEntry;
      ...
      for (GenericStack output : outputs(pattern)) {
   ...
   public static IPatternDetails findCompiledByOutput(AEKey outputKey) {
      ...
      // 1.19+ 走 outputKey.getId().toString() 拿 ResourceLocation.toString() 做 NBT-忽略 模式查找
