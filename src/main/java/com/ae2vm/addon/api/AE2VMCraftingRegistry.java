 ...
 * in {@code beginCraftingCalculation}) are left entirely to that mod's own
 * crafting logic — AE2 VM never intercepts them.
 *
 * <pre>{@code
...
public final class AE2VMCraftingRegistry {
    ...
    public static boolean isRegistered(String className) {
        ...
        for (String marker : REGISTERED) {
            if (className.contains(marker)) return true;
        }
        return false;
    }
    ...
    public static boolean isUnregisteredThirdParty(String className) {
        if (className == null) return false;
        if (className.startsWith("appeng.")) return false; // AE2's own → VM handles
