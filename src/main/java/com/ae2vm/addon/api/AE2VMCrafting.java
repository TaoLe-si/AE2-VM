...
public final class AE2VMCrafting {
    ...
    private static CompletableFuture<ICraftingPlan> calculateAsync(
        ...
        // PATTERNS candidate logging disabled (v1.8.20) — keep log clean.
        // logPatternCandidates(what, patterns);
    ...
    private static IPatternDetails resolve(Object network,
        ...
        if (!subs.isEmpty()) {
            // logPatternCandidates(key, subs); // disabled (v1.8.20)
    ...
    /**
     * DIAG (disabled v1.8.20): logs every candidate pattern for a key when there is
     * more than one, or when any candidate has a large per-craft output. Was used to
     * confirm the "mega" patterns are auto-generated by mekenergistics' ME machines /
     * smart pattern multiplication (physical ae2:processing_pattern items in the ME
     * machine's internal pattern slots, invisible in a normal pattern terminal).
     */
    // private static void logPatternCandidates(AEKey key, Collection<IPatternDetails> patterns) {
    //     boolean anyLarge = false;
    //     for (var p : patterns) {
    //         if (p == null) continue;
    //         var po = p.getPrimaryOutput();
    //         if (po != null && po.amount() > 1000) { anyLarge = true; break; }
    //     }
    //     if (!anyLarge && patterns.size() <= 1) return;
    //     var sb = new StringBuilder("[AE2-VM] PATTERNS(").append(key).append(") n=").append(patterns.size()).append(":");
    //     for (var p : patterns) {
    //         if (p == null) { sb.append(" [null]"); continue; }
    //         var po = p.getPrimaryOutput();
    //         sb.append(" [").append(po == null ? "?" : po.amount()).append("x")
    //           .append(po == null ? "?" : po.what())
    //           .append(" clazz=").append(p.getClass().getSimpleName());
    //         try {
    //             sb.append(" def=").append(p.getDefinition() == null ? "?" : p.getDefinition());
    //         } catch (Throwable t) {
    //             sb.append(" def=?");
    //         }
    //         sb.append("]");
