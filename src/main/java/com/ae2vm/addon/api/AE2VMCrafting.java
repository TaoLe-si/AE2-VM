...
// (v1.12.x GTL STALE-MISSING RETRY) Retry conditions:
                ...
                // (v1.13.4 MULTI-JOB STALL) DELIVERABILITY GUARD.
                ...
                // any mod that rebuilds its pattern provider after a config change)
                // re-encode their patterns into FRESH IPatternDetails instances. When the
                // provider refresh is batched/delayed, PatternCompiler.bumpPatternVersion()
                // has not fired yet when the next order is placed. Inside that window:
                //   * the memoized fast path replays the PREVIOUS plan verbatim -> its
                //     patternTimes keys are the OLD instances;
                //   * JIT bundles are reused as well, because patternsEquivalent() compares
                //     by CONTENT and a re-encoded instance looks "unchanged".
                // The plan therefore reports feasible while CraftingService.getProviders()
                // no longer knows those instances -> the CPU accepts the job, no provider
                // ever picks it up, and the job sits at 0% forever. Cancelling and
                // re-ordering works again because by then the refresh has landed, the
                // version was bumped, and a fresh capture binds the live instances.
                // Rebinding to the live instance (or forcing one cold re-capture when no
                // live equivalent exists) removes the stall at the root.
                ICraftingPlan deliverable = rebindStalePatterns(k -> service.getCraftingFor(k), rawPlan);
                if (deliverable == null) {
                    ...
                    // Throttled to one line per output key per 30s: it is a real fault worth
                    ...
                    try {
                        ...
                        ICraftingPlan coldChecked = rebindStalePatterns(k -> service.getCraftingFor(k), cold);
     ...
     */
    /**
     * (v1.13.4 MULTI-JOB STALL) Throttle for the {@code UNDELIVERABLE PLAN} warning:
    ...
    /**
     * (v1.13.4 MULTI-JOB STALL) Rebind every dispatched pattern to the instance the
    ...
    /**
     * (v1.13.4 MULTI-JOB STALL) Content-level comparison of two {@code IPatternDetails}.
