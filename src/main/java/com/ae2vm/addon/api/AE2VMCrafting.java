...
public final class AE2VMCrafting {
    ...
    /** (v1.15.x GTL WINDOW) How long a binary-search candidate waits for the GTL
     *  provider-refresh window to close when a missing key has a stale negative
     *  resolver entry (a query failed between removeProvider → addProvider). The
     *  game log shows the search collapsing to x7 while vanilla (1ms later) found
     *  x75 — the window closed right after the last candidate verdict. */
    private static final long CRAFT_LESS_WINDOW_POLL_MS = 500L;
...
//             }
    ...
    /**
     * (v1.15.x CRAFT_LESS) Binary search for the largest amount whose crafting plan
     * is completable (missingItems empty), mirroring AE2's
     * {@code CraftingCalculation.computePlan()} CRAFT_LESS path
     * (highestOneBit downward approximation, candidate &lt; requestedAmount).
     * <p>
     * Returns a completable plan for the best feasible amount, or {@code null} if
     * even the smallest candidate is infeasible — the caller then keeps the full
     * (simulate) plan which reports the missing items.
     */
    private static ICraftingPlan craftLessBinarySearch(
            IGrid grid,
            CraftingVM vm,
            CraftingService service,
            IPatternDetails topPattern,
            appeng.api.networking.storage.IStorageService storage,
            AEKey what,
            long amount) {
        long best = 0;
        ICraftingPlan bestPlan = null;
        long bit = Long.highestOneBit(amount);
        while (bit > 0) {
            long candidate = best + bit;
            if (candidate < amount) {
                ICraftingPlan candPlan = tryCraftLessCandidate(grid, vm, service, storage, topPattern, what, candidate);
                if (candPlan != null && candPlan.missingItems().isEmpty()) {
                    best = candidate;
                    bestPlan = candPlan;
                }
            }
    ...
    /**
     * (v1.15.x CRAFT-LESS STALE) Runs one binary-search candidate and applies the
     * same stale-negative recovery as the main flow's retry loop: a missing key that
     * NOW has a pattern was cached negative during an earlier capture (GTL provider
     * sync without a version bump). The main flow only clears the negative entries
     * for the FULL-size plan's missing keys; the binary search evaluates SMALLER
     * amounts whose missing-key set can differ (a key that the full plan never
     * needed), so it must run its own stale check or the whole search is poisoned
     * by the 2s negative TTL — the VM would report a tiny feasible amount while
     * vanilla's CRAFT_LESS finds a large one (game log: VM x2 vs VANILLA x29).
     *
     * <p>A candidate compile/execute crash is NOT an "infeasible" verdict — it is
     * recorded (was silently swallowed, which could also truncate the search).
     */
    private static ICraftingPlan tryCraftLessCandidate(
            IGrid grid,
            CraftingVM vm,
            CraftingService service,
            appeng.api.networking.storage.IStorageService storage,
            ...
            long candidate) {
        try {
            CraftingBytecode candBc = PatternCompiler.compileRequest(grid, topPattern, candidate);
            var candInv = new com.ae2vm.addon.vm.RealtimeNetworkCraftingSimulationState(storage);
            var candChild = new ChildCraftingSimulationState(candInv);
            candChild.ignore(what);
            ICraftingPlan candPlan = vm.execute(candBc, candChild);
            if (!candPlan.missingItems().isEmpty()) {
                boolean stale = staleMissingNowCraftable(service, vm, candPlan, what);
                boolean negCached = hasNegativeResolverEntries(vm, candPlan, what);
                if (stale || negCached) {
                    // (v1.15.x GTL WINDOW) A negative resolver entry exists for a
                    // missing key — one query failed during the GTL provider-refresh
                    // window (removeProvider → addProvider; getCraftingFor empty in
                    // between). Even when the key is NOT yet craftable at this instant
                    // the window may still be open: poll until it closes (or a
                    // deadline) before declaring the candidate infeasible. The game
                    // log shows the binary search collapsing to x7 while vanilla (1ms
                    // later) found x75 — the window closed right after the last
                    // candidate verdict, so the fixed 60ms settle was too short.
                    java.util.Map<AEKey, Object> rcache = vm.getResolverCache();
                    // Clear negatives that are craftable NOW.
                    for (var e : candPlan.missingItems()) {
                        AEKey missingKey = e.getKey();
                        if (missingKey == null || missingKey.equals(what)) continue;
                        Object cachedVal = rcache.get(missingKey);
                        if (cachedVal instanceof long[] && !vmCraftingFor(service, missingKey).isEmpty()) {
                    ...
                    // Poll for the provider-refresh window to close.
                    long deadline = System.currentTimeMillis() + CRAFT_LESS_WINDOW_POLL_MS;
                    ...
                    do {
                        ...
                        if (!windowClosed && System.currentTimeMillis() < deadline) {
                            try {
                                Thread.sleep(100L);
                            } catch (InterruptedException ie) {
                                Thread.currentThread().interrupt();
                    ...
                    } while (!windowClosed && System.currentTimeMillis() < deadline);
                    // (v1.15.x) The window may have closed WHILE we were polling — the
                    // negatives are now stale in the OTHER direction: clear every
                    // negative entry whose pattern is visible again, otherwise the
                    // retry's resolve() hits the 2s negative TTL and the candidate
                    // fails again (observed: search collapsed to 511/7 in tests/game).
                    for (var e : candPlan.missingItems()) {
                        AEKey missingKey = e.getKey();
                        if (missingKey == null || missingKey.equals(what)) continue;
                        Object cachedVal = rcache.get(missingKey);
                    ...
                    // Re-run the SAME candidate with the recovered pattern set.
                    CraftingBytecode reBc = PatternCompiler.compileRequest(grid, topPattern, candidate);
                    var reInv = new com.ae2vm.addon.vm.RealtimeNetworkCraftingSimulationState(storage);
                    var reChild = new ChildCraftingSimulationState(reInv);
                    reChild.ignore(what);
                    candPlan = vm.execute(reBc, reChild);
                }
            }
            return candPlan;
        } catch (Throwable t) {
//             AE2VMAddon.LOGGER.warn("[AE2-VM CRAFT-LESS] candidate x{} crashed: {}", candidate, String.valueOf(t));
            return null;
        }
    }
    /** True when any missing key carries a negative resolver entry — a query failed
     *  during a provider-refresh window (the pattern may still be invisible right now). */
    private static boolean hasNegativeResolverEntries(CraftingVM vm, ICraftingPlan plan, AEKey requested) {
        java.util.Map<AEKey, Object> rcache = vm.getResolverCache();
        for (var e : plan.missingItems()) {
            AEKey missingKey = e.getKey();
            if (missingKey == null || missingKey.equals(requested)) continue;
            if (rcache.get(missingKey) instanceof long[]) return true;
        }
        return false;
    }
