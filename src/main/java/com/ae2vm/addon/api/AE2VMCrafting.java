...
public final class AE2VMCrafting {
    ...
    /** v9 getCraftingFor(IAEStack) — shim key 转换后调用。 */
    private static Collection<IPatternDetails> craftingFor(CraftingService service, AEKey key) {
        if (key == null) return java.util.List.of();
        return service.getCraftingFor(((AEItemKey) key).toStack(1));
    }
    ...
    private static GenericStack wrapPrimary(IPatternDetails p) {
        ...
        return out instanceof IAEItemStack is ? GenericStack.wrap(is) : null;
    }
    ...
    private static GenericStack[] wrapOutputs(IPatternDetails p) {
        ...
        var out = new GenericStack[raw.length];
        for (int i = 0; i < raw.length; i++) {
            out[i] = raw[i] instanceof IAEItemStack is ? GenericStack.wrap(is) : null;
        }
        return out;
    }
...
// (v1.12.x GTL STALE-MISSING RETRY) Loop up to 3 times to handle stale
                ...
                for (int retry = 0; retry < maxRetries; retry++) {
                    ...
                    if (rawPlan.simulation() && !rawPlan.missingItems().isEmpty()) {
                        ...
                        if (missingCount > 0) {
                        var realStock = new com.ae2vm.addon.vm.RealtimeNetworkCraftingSimulationState(storage);
...
// (v1.12.x GTL STALE-MISSING RETRY) Retry conditions:
    ...
    private static KeyCounter stockForGrid(IGrid grid,
        ...
        try {
            var mon = storage.getInventory(appeng.api.storage.StorageChannels.items());
            if (mon != null) {
    ...
    public static boolean patternContentEquals(IPatternDetails a, IPatternDetails b) {
        ...
        try {
            var ao = wrapPrimary(a);
            var bo = wrapPrimary(b);
            ...
            for (int i = 0; i < ai.length; i++) {
                ...
                var ap = wrapPossible(ai[i]);
                var bp = wrapPossible(bi[i]);
    ...
    private static IPatternDetails resolve(Object network,
        ...
        var subs = craftingFor(service, key);
        if (!subs.isEmpty()) {
    ...
    private static boolean patternOutputs(IPatternDetails pattern, AEKey want) {
        ...
        if (want instanceof appeng.api.stacks.AEItemKey wItem
                && out.what() instanceof appeng.api.stacks.AEItemKey oItem) {
    ...
    public static java.util.Set<AEKey> computeCycleBoundKeys(
        ...
        try {
            ...
            if (candidate != null && candidate.getInputs() != null && candidate.getOutputs() != null) {
                ...
                if (!cins.isEmpty()) {
                    for (var gs : wrapOutputs(candidate)) {
    ...
    private static appeng.api.stacks.KeyCounter resolveStockSnapshot(Object network) {
        try {
            if (network instanceof IGrid g && g.getStorageService() != null) {
                ...
                var mon = g.getStorageService().getInventory(appeng.api.storage.StorageChannels.items());
                if (mon != null) {
                    for (IAEStack st : mon.getStorageList()) {
                        if (st instanceof IAEItemStack is) {
                            var k = AEItemKey.wrap(is);
    ...
    private static boolean staleMissingNowCraftable(CraftingService service, CraftingVM vm,
        ...
        for (var e : toKeyCounter(plan.missingItems()).entrySet()) {
