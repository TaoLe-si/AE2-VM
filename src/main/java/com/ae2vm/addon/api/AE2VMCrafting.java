...
public final class AE2VMCrafting {
    ...
    private static CompletableFuture<ICraftingPlan> calculateAsync(
        ...
        if (!vm.isExecuting()) {
            ...
            if (warmPlan != null) {
                // (v1.13.5 MULTI-JOB STALL) DELIVERABILITY GUARD on the warm path.
                ...
                ICraftingPlan warmChecked = rebindStalePatterns(k -> service.getCraftingFor(k), warmPlan);
                if (warmChecked != null) {
        ...
        return CompletableFuture.supplyAsync(() -> {
            try {
                ...
                ICraftingPlan warmPlan = tryWarmPlan(vm, grid, storage, topPattern, amount, what);
                if (warmPlan != null) {
                    // (v1.13.5 MULTI-JOB STALL) Same deliverability guard as the
...
// (v1.12.x GTL STALE-MISSING RETRY) Retry conditions:
                ...
                // (v1.13.5 MULTI-JOB STALL) DELIVERABILITY GUARD.
    ...
    /**
     * (v1.13.5 MULTI-JOB STALL) Throttle for the {@code UNDELIVERABLE PLAN} warning:
    ...
    /**
     * (v1.13.5 MULTI-JOB STALL) Rebind every dispatched pattern to the instance the
    ...
    /**
     * (v1.13.5 MULTI-JOB STALL) Content-level comparison of two {@code IPatternDetails}.
    ...
    private static IPatternDetails resolve(Object network,
        ...
        if (id != null) {
            ...
            // (MC 26) BuiltInRegistries.ITEM.get(Identifier) returns Optional<Holder.Reference<Item>>;
            // 1.21.1 returned Item directly. Use getValue(id) which returns T or null.
