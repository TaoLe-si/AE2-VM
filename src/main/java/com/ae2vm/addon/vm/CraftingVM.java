...
public class CraftingVM {
    ...
    private static appeng.api.stacks.GenericStack wrapPrimary(IPatternDetails p) {
        ...
        return out instanceof IAEItemStack is ? appeng.api.stacks.GenericStack.wrap(is) : null;
    }
    ...
    private static appeng.api.stacks.GenericStack[] wrapOutputs(IPatternDetails p) {
        ...
        var out = new appeng.api.stacks.GenericStack[raw.length];
        for (int i = 0; i < raw.length; i++) {
            out[i] = raw[i] instanceof IAEItemStack is ? appeng.api.stacks.GenericStack.wrap(is) : null;
        }
        return out;
    }
    ...
    private static KeyCounter toKeyCounter(MixedStackList list) {
        ...
        for (IAEStack st : list) {
            if (st instanceof IAEItemStack is) {
                var k = appeng.api.stacks.AEItemKey.wrap(is);
    ...
    private Map<AEKey, Long> computeFeedbackLoopMissing(Map<AEKey, BigInteger> total,
        ...
        for (var en : total.entrySet()) {
            ...
            var outs = wrapOutputs(details); // v9: IAEStack[] → wrapOutputs
            if (outs != null) {
                int outIdx = 0;
            ...
            for (IPatternDetails details : patterns) {
                ...
                if (details.getInputs() != null) {
                    for (IPatternDetails.IInput entry : details.getInputs()) {
                        var possible = wrapPossible(entry);
                        if (possible == null || possible.length == 0 || possible[0] == null
                                || possible[0].what() == null) continue;
                        AEKey ik = possible[0].what();
                        // AE2 1.18.1 (forge/v10.x): IInput.getRemainingKey(AEKey) 不存在
        ...
        if (realStockCache == null) {
            ...
            try {
                if (networkKey instanceof appeng.api.networking.IGrid g) {
                    ...
                    if (st != null) {
                        var mon = st.getInventory(appeng.api.storage.StorageChannels.items());
                        if (mon != null) {
    ...
    private static boolean isUnseededSelfLoop(IPatternDetails pattern) {
        ...
        for (var input : inputs) {
            var possible = wrapPossible(input);
