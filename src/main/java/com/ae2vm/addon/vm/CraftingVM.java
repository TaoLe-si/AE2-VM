...
public class CraftingVM {
    ...
    private void applyBundleDirect(Bundle b) {
        ...
        for (java.util.Map.Entry<com.ae2vm.shim.api.stacks.AEKey, java.math.BigInteger> e : b.used.entrySet()) {
            ...
            // 1.12.2-nova 诊断：缺额记在 b.used 的差额上还是 b.missing 上，是两条完全不同的
            // 归因路径（前者=聚合阶段的需求算错，后者=捕获阶段留下的旧账），必须一眼分开。
            if (shortfall > 0 && com.ae2vm.addon.config.AE2VMConfig.isDebugLogging()) {
                AE2VMAddon.LOGGER.info("[AE2-VM] SHORTFALL(used) bundle=" + bundleTag(b) + " key=" + e.getKey()
                        + " demand=" + val + " got=" + got + " fromInternal=" + fromInternal
                        + " fromNetwork=" + fromNetwork + " shortfall=" + shortfall);
            }
        }
        ...
        for (java.util.Map.Entry<com.ae2vm.shim.api.stacks.AEKey, java.math.BigInteger> e : b.seeds.entrySet()) {
            ...
            if (shortfall > 0) missingItems.add(e.getKey(), shortfall);
            if (shortfall > 0 && com.ae2vm.addon.config.AE2VMConfig.isDebugLogging()) {
                AE2VMAddon.LOGGER.info("[AE2-VM] SHORTFALL(captured-missing) bundle=" + bundleTag(b)
                        + " key=" + e.getKey() + " recorded=" + val + " gotNow=" + got
                        + " shortfall=" + shortfall);
            }
        }
...
//                         AE2VMAddon.LOGGER.warn("[AE2-VM]   → CALL_BY_KEY {} req={} → CYCLE, consuming available stock", tk, req);
    ...
    private void logPlanResult(ICraftingPlan plan, long vmStartNs) {
        ...
        // 1.12.2-nova 诊断：把每张被派发的样板"内核看到的量/候选"原样打出来 ——
        // §22 的量纲契约是 per-craft = multiplier × possibleInputs[0].amount()，
        // 而 1.12.2 的 meta 变体（planks:0..5 是同一个 Item）要求候选必须列全，
        // 所以这里把 sub/cand 的全部 meta 都打出来，看一行就能定，不用再猜。
        if (com.ae2vm.addon.config.AE2VMConfig.isDebugLogging()) {
            for (java.util.Map.Entry<com.ae2vm.shim.api.crafting.IPatternDetails, Long> e : this.patternTimes.entrySet()) {
                com.ae2vm.shim.api.crafting.IPatternDetails pd = e.getKey();
                if (pd == null) { continue; }
                appeng.api.networking.crafting.ICraftingPatternDetails raw =
                        com.ae2vm.addon.v8.V8PatternDetails.unwrap(pd);
                StringBuilder io2 = new StringBuilder();
                if (pd.getOutputs() != null) {
                    for (appeng.api.storage.data.IAEStack o : pd.getOutputs()) {
                        if (o == null) { continue; }
                        io2.append('[').append(describeVm(o)).append("] ");
                    }
                }
                StringBuilder ii = new StringBuilder();
                if (pd.getInputs() != null) {
                    for (com.ae2vm.shim.api.crafting.IPatternDetails.IInput in : pd.getInputs()) {
                        if (in == null) { continue; }
                        appeng.api.storage.data.IAEStack[] cand = in.getPossibleInputs();
                        ii.append("[mult=").append(in.getMultiplier())
                          .append(" cand=").append(cand == null ? -1 : cand.length);
                        if (cand != null) {
                            for (appeng.api.storage.data.IAEStack c : cand) {
                                ii.append(' ').append(describeVm(c));
                            }
                        }
                        ii.append("] ");
                    }
                }
                // "样板重编译后是不是坏了"的直接证据：把这份样板物品**身上的原始 NBT**
                // （rv6/uel 的 PatternHelper 构造期就是吃它，in/out/crafting/substitute/wide 全在这）
                // 连实例 hash 一起打出来 —— 编译进来的量与 AE2 现在认的量对不对得上，看这一行就够。
                String rawNbt = "?";
                if (raw != null) {
                    try {
                        net.minecraft.item.ItemStack pat = raw.getPattern();
                        net.minecraft.nbt.NBTTagCompound tag = pat == null ? null : pat.getTagCompound();
                        rawNbt = tag == null ? "null" : tag.toString();
                        if (rawNbt.length() > 400) {
                            rawNbt = rawNbt.substring(0, 400) + "…";
                        }
                    } catch (Throwable t) {
                        rawNbt = "<" + t.getClass().getSimpleName() + ">";
                    }
                }
                AE2VMAddon.LOGGER.info("[AE2-VM] PATTERN times=" + e.getValue()
                        + " sub=" + (raw == null ? "?" : Boolean.valueOf(raw.canSubstitute()))
                        + " id=" + System.identityHashCode(raw)
                        + " outs=" + io2 + " ins=" + ii);
                AE2VMAddon.LOGGER.info("[AE2-VM] PATTERN NBT " + rawNbt);
            }
            // 报缺的每一项：把内核自己那几本账原样打出来（不改任何计算，只是读字段）。
            // 恒等式应当是 demand = usedFromStock + simInserted(自有产出折抵) + missing，
            // 哪一项不对一眼可见。stockAtStart 是 execute() 开头仿真库存的快照（计划依据的数），
            // stockNow 是这次取用的实时快照 —— 两者不等就说明网络库存正在被别的 CPU 抽走。
            for (IAEStack ms : plan.missingItems()) {
                if (ms == null) { continue; }
                com.ae2vm.shim.api.stacks.AEItemKey mk = ms instanceof appeng.api.storage.data.IAEItemStack
                        ? com.ae2vm.shim.api.stacks.AEItemKey.wrap((appeng.api.storage.data.IAEItemStack) ms)
                        : null;
                if (mk == null) { continue; }
                long inserted = simInternal == null ? -1 : simInternal.get(mk);
                long atStart = executeStartStock == null ? -1 : executeStartStock.get(mk);
                AE2VMAddon.LOGGER.info("[AE2-VM] MISSING DETAIL " + describeVm(ms)
                        + " missing=" + missingItems.get(mk)
                        + " usedFromStock=" + usedItems.get(mk)
                        + " planEmitted=" + emittedItems.get(mk)
                        + " simInserted=" + inserted
                        + " stockAtStart=" + atStart
                        + " stockNow=" + realStockOf(mk)
                        + " fuzzyNow=" + fuzzyStockNow(mk));
            }
        }
    }

    /** 诊断用：实时快照里"同一 Item 的任意 damage/NBT 变体"合计（FuzzyMode.IGNORE_ALL 口径）。 */
    ...
    /** 诊断用：物品注册名 + damage + 数量（{@code toString()} 不含 damage，看不出 meta 变体）。 */
    private static String describeVm(appeng.api.storage.data.IAEStack s) {
        if (s == null) {
            return "null";
        }
    ...
    /** 诊断用：这个 bundle 是哪张样板（连它自己被派工的次数）—— 短额记在哪个 bundle 上。 */
    private static String bundleTag(Bundle b) {
        StringBuilder sb = new StringBuilder();
        try {
            for (java.util.Map.Entry<IPatternDetails, java.math.BigInteger> e : b.patterns.entrySet()) {
                if (sb.length() > 0) {
                    sb.append(", ");
                }
        ...
        return sb.length() == 0 ? "(no-pattern)" : sb.toString();
    }
