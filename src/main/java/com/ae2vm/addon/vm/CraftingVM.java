...
public class CraftingVM {
    ...
    private Map<AEKey, Long> computeFeedbackLoopMissing(Map<AEKey, BigInteger> total,
        ...
        for (var en : total.entrySet()) {
            ...
            var outs = details.getOutputs(); // List<GenericStack> on 1.21.1
            if (outs != null) {
                int outIdx = 0;
    ...
    private static boolean patternsEquivalent(IPatternDetails a, IPatternDetails b) {
        ...
        var ao = a.getOutputs();
        var bo = b.getOutputs();
        if (ao == null || bo == null || ao.size() != bo.size()) return false;
        for (int i = 0; i < ao.size(); i++) {
            if (!stacksEqual(ao.get(i), bo.get(i))) return false;
        }
    ...
    private void logPlanResult(ICraftingPlan plan, long vmStartNs) {
        ...
        if (!patternTimes.isEmpty()) {
            ...
            for (var e : sorted) {
                if (i > 0) sw.write(", ");
                String name = "?";
                var outs = e.getKey().getOutputs(); // List<GenericStack> on 1.21.1
                if (outs != null && !outs.isEmpty() && outs.get(0) != null && outs.get(0).what() != null) {
                    name = outs.get(0).what().toString();
                }
                sw.write(name);
                sw.write("=");
