...
public class JitReuseTest {
    ...
    void reuseVmAcrossRequests() throws Exception {
        ...
        for (int i = 0; i < 5; i++) {
            ...
            System.out.println("[JIT-REUSE] request=" + i
                    + " sim=" + plan.simulation()
                    + " usedD=" + BenchCompat.usedOf(plan, BenchAEKey.of("D"))
                    + " usedE=" + BenchCompat.usedOf(plan, BenchAEKey.of("E"))
                    + " usedF=" + BenchCompat.usedOf(plan, BenchAEKey.of("F"))
                    + " usedG=" + BenchCompat.usedOf(plan, BenchAEKey.of("G"))
                    + " missing=" + plan.missingItems());
            Assertions.assertTrue(plan.missingItems().isEmpty(), "no missing on request " + i);
            Assertions.assertEquals(4L, BenchCompat.usedOf(plan, BenchAEKey.of("D")));
            Assertions.assertEquals(4L, BenchCompat.usedOf(plan, BenchAEKey.of("E")));
            Assertions.assertEquals(4L, BenchCompat.usedOf(plan, BenchAEKey.of("F")));
            Assertions.assertEquals(4L, BenchCompat.usedOf(plan, BenchAEKey.of("G")));
        }
    ...
    void reuseVmWithLaterShortfall() throws Exception {
        ...
        System.out.println("[JIT-REUSE-SHORT] sim=" + shortPlan.simulation()
                ...
                + " usedD=" + BenchCompat.usedOf(shortPlan, BenchAEKey.of("D")));
        ...
        Assertions.assertEquals(2L, BenchCompat.usedOf(shortPlan, BenchAEKey.of("D")),
                "D used should be the 2 actually extractable");
        Assertions.assertEquals(2L, BenchCompat.missingOf(shortPlan, BenchAEKey.of("D")),
    ...
    void reuseVmMustRefreshStockSnapshot() throws Exception {
        ...
        Assertions.assertTrue(ok.missingItems().isEmpty(), "first request should be feasible");
        Assertions.assertEquals(4L, BenchCompat.usedOf(ok, BenchAEKey.of("D")));
        ...
        System.out.println("[JIT-REUSE-STOCK] req2 usedD=" + BenchCompat.usedOf(plan2, BenchAEKey.of("D"))
                + " missing=" + plan2.missingItems() + " sim=" + plan2.simulation());
        Assertions.assertEquals(2L, BenchCompat.usedOf(plan2, BenchAEKey.of("D")),
        ...
        Assertions.assertEquals(2L, BenchCompat.missingOf(plan2, BenchAEKey.of("D")),
    ...
    void reuseVmMustReReadStockForCraftableSubItem() throws Exception {
        ...
        System.out.println("[REUSE-B-STOCK] req1 usedB=" + BenchCompat.usedOf(plan1, B)
        ...
        Assertions.assertEquals(4L, BenchCompat.usedOf(plan1, B), "req1: B taken from stock");
        ...
        ICraftingPlan plan2 = vm.execute(PatternCompiler.compileRequest(byOutput.get(A), 4),
                new BenchSimulationState(stock));
        System.out.println("[REUSE-B-STOCK] req2 usedB=" + BenchCompat.usedOf(plan2, B)
        ...
        for (java.util.Map.Entry<String, Long> e : BenchCompat.missing(plan2).entrySet()) {
            missDetail.append(" ").append(e.getValue()).append("x").append(e.getKey());
        }
        System.out.println(missDetail);
        StringBuilder usedDetail = new StringBuilder("[REUSE-B-STOCK] req2 USED DETAIL:");
        for (java.util.Map.Entry<String, Long> e : BenchCompat.used(plan2).entrySet()) {
            usedDetail.append(" ").append(e.getValue()).append("x").append(e.getKey());
        }
        System.out.println(usedDetail);
        Assertions.assertEquals(2L, BenchCompat.usedOf(plan2, B),
