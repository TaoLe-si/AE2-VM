...
![Minecraft](https://img.shields.io/badge/Minecraft-26.1.2-blue?logo=minecraft)
![NeoForge](https://img.shields.io/badge/NeoForge-26.1.2.94+-orange)
![AE2](https://img.shields.io/badge/AE2-26.1.10--beta+-green)
...
**AE2 VM** 是 [Applied Energistics 2](https://github.com/AppliedEnergistics/Applied-Energistics-2) 的 NeoForge 扩展模组（本分支适配 **MC 26.1.2 / NeoForge 26.1.2**）。它将 AE2 原本的**递归合成树遍历**替换为**栈式虚拟机（Stack-based VM）执行编译后的字节码**，实现合成计算的极限加速。适用于深度嵌套的大型合成配方（如 AE2 扩展包中的无限存储元件）。
...
1. 安装 [NeoForge 26.1.2](https://neoforged.net/) 与 [Applied Energistics 2 26.1.10-beta+](https://www.curseforge.com/minecraft/mc-mods/applied-energistics-2)
2. 将 `ae2vm-1.10.7_neoforge_26.1.2.jar` 放入 `mods/` 文件夹（请删除旧版本 jar，仅保留最新版）
   - 无针对检测版请使用 `ae2vm-nodetect-1.10.7_neoforge_26.1.2.jar`（检测到不兼容作者 mod 时只警告、不闪退）
...
│   └── AE2VMConfig.java             # 配置入口（ModConfigSpec → config/ae2vm-common.toml，proxyEnabled 开关）
├── mixin/
...
| Mod ID | `ae2vm` |
| 名称 | AE2 VM |
| 版本 | 1.10.7（jar 后缀 `_neoforge_26.1.2`） |
...
| Minecraft | 26.1.2 |
| NeoForge | ≥ 26.1.2.94 |
| Applied Energistics 2 | ≥ 26.1.10-beta |
...
```bash
set JAVA_HOME=D:\Java26   # 需要 Java 26 / 25（Gradle 9.7.0）
...
# 输出（<ver> = 当前版本，如 1.10.7；带 _neoforge_26.1.2 后缀）
# build/libs/ae2vm-<ver>_neoforge_26.1.2.jar            （针对检测版）
# build/libs/ae2vm-nodetect-<ver>_neoforge_26.1.2.jar   （无针对检测版）
```
