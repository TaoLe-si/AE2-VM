...
- **版本**: AE2VMAddon **1.10.2**（MC 1.20.1 / Forge 47.4.22，JAVA_HOME=D:\Java21）
- **引擎**: AE2VM `CraftingVM`
...
## 总览（1.21.1 主版本测试计数，来自 JUnit XML 报告）

```
测试类: 16    用例: 119    失败: 0    错误: 0    跳过: 0
构建: BUILD SUCCESSFUL
闪电基准（18:58 全量运行）: cases=33 supported=28 falsePositive=5 engineError=0 timeout=0 totalElapsedMs=80.3
...
## 二、边界能力基准（37 例场景 / 38 用例，1.21.1 主版本实际运行）
...
催化剂（9/9）与耐久（3/3）能力族本轮全部修复并同步到 1.20.1；处理配方默认模糊修复落地。
全部 119 个 VM/闪电/边界/回归用例 0 失败，VM 计算全部 < 4.2 ms/例（数据来自 1.21.1
同引擎运行）。
