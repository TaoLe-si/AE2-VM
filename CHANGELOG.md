...
## [1.13.4] - 2026-09-15 (多笔订单卡死：样板实例失效 → 计划不可投递)
  ...
  - **根因**：样板机器（ME 样板总成 / `MESuperPatternBufferPartMachine` FOA 模式 /
    ...
    **重新编码为全新的 `IPatternDetails` 实例**。当 provider 刷新被批量延迟时，
    ...
    - 记忆化快路径 `CraftingVM.tryFastPath` **逐字重放上一笔计划**，
  ...
  - **修复**：
    - `AE2VMCrafting.rebindStalePatterns(liveLookup, plan)`（新增）：投递前把每个
      已派发样板重新绑定到网络当前实例。无等价 live 实例则判不可投递并冷重算一次
      （只清本 VM 的 `bundleCache`/`resolverCache`，**不做全局 `bumpPatternVersion`**，
      避免把同网格其他 VM 推进 removeProvider→addProvider 窗口）。
      `getCraftingFor` 为空视为 provider 刷新窗口，原样返回不误判。
    - `AE2VMCrafting.patternContentEquals(a, b)`（新增）：内容级比较，用于识别
      「实例已换但内容相同」的重编码样板。
    ...
    - `CraftingVM.clearBundleCache()` 同时丢弃快路径记忆
      （`fastPlanKey/Patterns/Used/Missing/Emitted`）：此前只清 `bundleCache`，
      `fastPlanPatterns` 仍持有旧实例 → 计划依旧不可投递。
  - **构建**：删除 `AE2VMConfigData.java` / `AE2VMConfigImpl.java` 两个 Cloth Config
    遗留文件（无任何引用，且 `me.shedaniel.autoconfig` 依赖缺失导致离线编译失败）。
    该版本此前**离线编译即失败**，现已可构建。
