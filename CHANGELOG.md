# AE2 VM Addon — 1.16.2 Forge 变更日志
## v1.14.2 (MC 1.16.2) — 2026-09-21（VM 沙箱补上真库存：修"自造中间品被报成缺料"）
...
## v1.14.1 (1.16.2-forge) — 2026-09-21（移植到 MC 1.16.2 / AE2 v8.1.0）

移植自 **1.16.1 fork**（`git archive` 取 HEAD `3839ae7`），AE2 用 **v8.1.0**（`8.1.x-1.16.x`）。

**为什么 1.16.2 也用 8.1.0**：AE2 官方没为 1.16.2 单独发版 —— v8.1.0 自己 mods.toml 写的就是
`forge [33.0.22,36.0.0)` + `mc [1.16.2,1.17.0)`，即同一份 jar 同时覆盖 1.16.2 与 1.16.3；
而"有没有 `ICraftingPatternDetails.getSubstituteInputs(int)`"这类 API 面由 **AE2 jar** 决定
（8.2.0 才加），与 MC 小版本无关 → 与 1.16.3 一样**零业务代码改动**。

**Forge 版本实测选定**：1.16.2 属 Forge **33.0.x** 线（34.x 才是 1.16.3）。
maven 逐个 HEAD：`1.16.2-33.0.61` 有 userdev、`33.0.62` 与 `34.1.33` 对 1.16.2 都是 404 →
取 **33.0.61**（1.16.2 线上最后一个）。若实机实例是别的 33.0.x，按 1.16.5 fork 的先例改成实例实版。

**构建坐标**：`minecraft_version=1.16.2`（range 钉死 `[1.16.2]`）、`forge_version=33.0.61`
（range `[33.0.22,34.0.0)`，下限取 AE2 8.1.0 自己声明的值）、`mcp_mappings=1.16.2`（official 通道）、
`ae2_version=8.1.0`、`libs/appliedenergistics2-8.1.0.jar`。

**踩坑复现**：首次 `build` 报 `Could not find net.minecraftforge:forge:1.16.2-33.0.61_mapped_official_1.16.2`
（只搜了 `bundeled_repo`）—— 就是 MIGRATION-PATTERNS §17 的共享守护进程陈旧坑（本项目里第三次遇到），
`gradlew --stop` 后一次通过。

**验证状态**：BUILD SUCCESSFUL、VMTest 6/6 PASSED、47 个 class（`appeng/` 0、mixin 3）、
Java 8 字节码、SRG 名命中 7、sha1 `549a3aa0f90d`；
`V8PatternDetails`/`VMCraftingJob`/`V8StorageService`/`CraftingGridCacheMixin` 与 1.16.1、1.16.3 **逐字节相同**。
实例已就位：`E:/MC/.minecraft/versions/1.16.2-Forge_33.0.61`（profile json 实测 `net.minecraftforge:forge:1.16.2-33.0.61`，与我编译用的构建号一致，所以不用像 1.16.3 那样再改钉）。
jar 已部署，`build/libs` 与实例 `mods/` 两份 sha1 一致 `06aeff15e700`（168824 字节）。`libs/` 也换成实例那份重签名 AE2 jar，然后 `compileJava`/`jar` 双双 UP-TO-DATE —— Gradle 的编译类路径默认忽略 `META-INF`，这本身就证明"重签名与否不影响编译"。

⚠️ 顺手验了一个 FG5 的隐患：`jar` UP-TO-DATE 而 `reobfJar` 重跑时，它的输入是**上一次已经 reobf 过**的 jar（reobf 是 in-place 覆盖），等于做了第二遍。SpecialSource 的 MCP→SRG 是幂等的（SRG 名不在映射左列），实测 8 个关键 class（含 3 个 mixin、`CraftingVM`、`PatternCompiler`）在 1.16.1 / 1.16.2 / 1.16.3 三份**部署件**里逐字节相同，且全量条目比对 **class 差异 0**（只有 `mods.toml`/`mixins.json`/`pack.mcmeta` 3 个资源不同）。但整体 jar 的 sha1 会变（条目时间戳），所以**跨版本比对要用逐条目 CRC，不要比整体 sha1**。**实机未验**：需要进世界跑一次合成下单（工作台 下单 1 → 4 木板；下单 10 → 40）。
