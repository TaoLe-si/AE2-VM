# AE2 VM Addon — 1.16.3 Forge 变更日志
## v1.14.2 (MC 1.16.3) — 2026-09-21（VM 沙箱补上真库存：修"自造中间品被报成缺料"）
...
## v1.14.1 (1.16.3-forge) — 2026-09-21（移植到 MC 1.16.3 / AE2 v8.1.0）

移植自 **1.16.1 fork**（AE2 v8.0.0），对应 AE2 官方 **`8.1.x-1.16.x`** 分支（tag `v8.1.0`：
`minecraft_version=1.16.3`、`forge_version=34.1.33`、mods.toml 声明 forge `[33.0.22,36.0.0)` /
mc `[1.16.2,1.17.0)`）。

**为什么基座选 1.16.1 而不是更新的 1.16.4**：javap 逐类差分（AE2-refs 里 8.0.0 / 8.1.0 / 8.2.0 三份 jar）
- 8.0.0 vs 8.1.0：9 个关键类（含三个挂载点 `CraftingGridCache` /
  `cluster.implementations.CraftingCPUCluster` / `DualityInterface`）**成员差异 0**；
- 8.1.0 vs 8.2.0：只差 `ICraftingPatternDetails.getSubstituteInputs(int)`（**8.2.0 才有**）。

而 1.16.1 那份 `V8PatternDetails` 正是按"没有 `getSubstituteInputs`"写的（候选输入退化单键，
MIGRATION-PATTERNS §19.2），所以 **本 fork 零业务代码改动**，只换构建坐标。已核对：
产物里 `V8PatternDetails.class`/`VMCraftingJob.class`/`V8StorageService.class`/
`CraftingGridCacheMixin.class` 与 1.16.1 **逐字节相同**（`f9d77ab83862` / `2ad9cdba2830` /
`c3d568822791` / `d56140beb044`）。

**构建坐标改动**：`minecraft_version=1.16.3`（range `[1.16.3]`）、`forge_version=34.1.42`
（= 用户实机实例版本；AE2 官方用的是 34.1.33）、`mcp_mappings=1.16.3`（official 通道）、
`ae2_version=8.1.0`、`libs/appliedenergistics2-8.1.0.jar`。

**编译期踩到并已复现 §17**：换成 34.1.42 后首次构建报
`Could not find net.minecraftforge:forge:1.16.3-34.1.42_mapped_official_1.16.3`
（只搜了 `bundeled_repo`，说明产 mapped 工件的 provider 没挂上）——
`gradlew --stop` 重启守护进程后一次通过，不是坐标写错。

**编译依赖用的是实例那份 jar**：实例里那份（CurseForge）与 modmaven 那份差 1 字节，
逐条目比对后差异只有 `META-INF/MANIFEST.MF` / `APPLIEDE.SF` / `APPLIEDE.RSA`
（CurseForge 重签名），3588 个条目 CRC 全同、7 个关键类 javap 全同 → 直接拿实例那份进 `libs/`，
保证"编译所用 = 运行所在"。

**验证状态**：BUILD SUCCESSFUL、VMTest 6/6 PASSED、47 个 class（`appeng/` 0、mixin 3）、
Java 8 字节码、SRG 名命中 7；`build/libs` 与实例 `mods/` 两份 sha1 一致 `2da777e1ad50`
（168824 字节）。**实机未验** —— 需要进世界跑一次合成下单，
用例：工作台 下单 1 → 期望消耗 4 木板；下单 10 → 期望 40。
