# AE2 VM Addon — 1.16.4 Forge 变更日志
## v1.14.2 (MC 1.16.4) — 2026-09-21（VM 沙箱补上真库存：修"自造中间品被报成缺料"）
...
移植自 **1.16.5 fork**（`429876d` → `35bf858`，AE2 v8.4.7），对应 AE2 官方 **`8.2.x-1.16.x`** 分支。

---

## v1.14.1+planfix (1.16.4-forge) — 2026-09-20（同步 1.16.5 已实机验证的合成计划表修复）

1.16.5 上实机确认「合成计划表看不到要合成的最终产物数量 / 产物数量不对」后，把同一份
`VMCraftingJob.java` 同步进本 fork（三个 fork 的该类此前 sha1 完全相同 `48cbc29d78d7`，
即同一个 bug 三版共有；同步后三版 `VMCraftingJob.class` 均为 `2ad9cdba2830`）。

根因（反编译 `appliedenergistics2-forge-15.4.10.jar` 的
`appeng.menu.me.crafting.CraftingPlanSummary.fromJob` 核实，1.20.1 baseline 界面读的就是它）：

| VM 变量 | baseline 里的用处 |
|---|---|
| `usedItems[key]` | → stored 基数 |
| `missingItems[key]` | → stored 基数（差额由 SIMULATE 抽取算出） |
| `emittedItems[key]` | → stored **和** crafting |
| `patternTimes[p] × p.getOutputs()` | → `crafting += out.amount × times`，**"要合成数量"的唯一来源** |
| `finalOutput` | 界面**不读**；只是 CPU 的交付目标 |

v8 的 `populatePlan` 只搬了 `used/missing/emitted`，**漏掉 `patternTimes → crafting` 那一路**，
所以产物行整个不出现。修复即按上表把四个变量摊进 v8 的两个载体
（`stackSize` = stored 基数，`countRequestable` = crafting），数值全部取 VM 已算出的结果。

> ⚠️ 本 fork 之前记的「原料扣取不对」很可能就是这张表漏行造成的误判 ——
> 中间产物行（例如 木板）整行不显示，看起来就像料被多吃/少吃。
> 1.16.4 的 AE2 侧与本 fork 逐字节一致（`CraftingPatternDetails` 920 行 0 差异），
> 所以**先按修复版重测**，再决定要不要去追 8.2.0→8.4.7 那三处 `CraftingCPUCluster`
> （`storeItems` / `completeJob` / `injectItems`）的真实语义差异。

构建：`gradlew build copyJarToMods --offline` → BUILD SUCCESSFUL，VMTest 6/6 PASSED，
jar 已落 `1.16.4-Forge_35.1.37/mods`。**仍未实机验证。**

---

## v1.14.1 (1.16.4-forge) — 2026-09-20

### 版本矩阵

| 项 | 值 | 依据 |
|---|---|---|
| MC | 1.16.4 | 实例 `E:/MC/.minecraft/versions/1.16.4-Forge_35.1.37` |
| Forge | 编译 `35.1.37` / 声明 `[35.1.4,36.0.0)` | 编译目标 = 实机 build（1.16.5 的错位教训）；区间照抄 AE2 8.2.0 jar 内 mods.toml |
| AE2 | 8.2.0（`libs/appliedenergistics2-8.2.0.jar`） | jar 内 `Implementation-Version: 8.2.0`、`forge=[35.1.4,36.0.0)`、`minecraft=[1.16.2,1.17.0)` |
| mappings | `official / 1.16.4` | piston-meta 实测 1.16.4 有 `client_mappings`（sha1 `0837de81…2f0e`），runtime `jre-legacy` = Java 8 |
| loaderVersion | `[32,)` 不变 | AE2 8.2.0 jar 内 mods.toml 本身就是 `[32,)` |
| 工具链 | FG 5.1.69 + Gradle 7.6.4 + JDK 8 daemon，测试 fork JDK 17 | 与 1.16.5 fork 一致 |

### 业务代码：零改动

移植前用 `javap` 把本 fork 全部 **27 个 `appeng.*` 依赖点**（3 个 mixin 目标 + v8 桥接 + shim 用到的 API）
在 8.0.0 / 8.2.0 / 8.4.7 三个 jar 里逐个比过，v8.2.0 相对 v8.4.7 只有 3 处差异，且**都不在本 fork 的调用路径上**：

| 差异 | 是否影响 1.16.4 |
|---|---|
| `ICraftingPatternDetails.getSubstituteInputs(int)`：8.0.0 **无**，8.2.0/8.4.7 有 | 无影响，`V8PatternDetails.java:116` 照常编 |
| `DualityInterface.isRemote()`：仅 8.4.7 有 | 本 fork 不调用 |
| `DualityInterface.hasCapability(...)`：仅 8.0.0/8.2.0 有 | 本 fork 不调用 |

三个挂载点（`CraftingGridCache.beginCraftingJob` / `CraftingCPUCluster.submitJob` /
`DualityInterface.updateCraftingList`）签名在 8.2.0 与 8.4.7 **逐字一致** → mixin、shim、VM
全链路一行没改，首轮 `gradlew clean build` 直接通过。

### 顺带修掉一个真实漏洞：`useJUnitPlatform()` 缺失

1.16.5 fork 的 `build.gradle` 没有 `useJUnitPlatform()`，而 `VMTest` 用的是 JUnit 5
（`org.junit.jupiter.api.Test`）。缺这行时 Gradle 退回 JUnit 4 provider，**发现 0 个测试仍然报
BUILD SUCCESSFUL** —— 即 1.16.5 记录的"VMTest 6/6 通过"用仓库里的脚本复现不出来
（`build/test-results` 里连 XML 都没有）。

普查所有 fork：1.17.1 / 1.18.1 / 1.18.2 / 1.19.2 / 1.19.3 / 1.20.1 / 1.21.1 / 26.1 **都有**这行，
只有 1.16.5 在"为 FG5 + Java 8 重写 build.gradle"时漏掉了。

本 fork 已补上，实测 6 条用例真跑起来并全 PASSED：
`TEST-com.ae2vm.addon.VMTest.xml` → `tests=6 failures=0 errors=0 skipped=0`
（`testSimpleBytecode` / `testFullPatternBytecode` / `testDivRoundUp` / `testMultiplication` /
`testRequestWrapperBytecode` / `testCallByKeyBytecode`）。

### 构建与产物核对

```
gradlew.bat clean build copyJarToMods     （在线：1.16.4 userdev / mojmap 首轮需下载）
BUILD SUCCESSFUL in 4m 34s
```

产物 jar 已核对：47 个 class、**class major 52（Java 8）**、`Manifest` 带
`MixinConfigs: ae2vm.mixins.json`、`appeng/` 开头 class 数 == 0（铁律 3）、
mods.toml 展开为 `loaderVersion="[32,)"` / `forge=[35.1.4,36.0.0)` /
`minecraft=[1.16.2,1.17.0)` / `appliedenergistics2=[8.2.0,9)`。
已落到 `E:/MC/.minecraft/versions/1.16.4-Forge_35.1.37/mods/ae2vm-1.14.1_forge_1.16.4.jar`。

### 仍未做的验证

编译 + 单测 + 字节码/元数据核对都过了，但**还没实机进世界**跑过一次合成请求接管
（mixin 是否真在 1.16.4 命中、`vmBeginCraftingJob` 是否被调用）。
1.16.5 是靠 5 次实机崩溃才把问题挖干净的，这一步不能拿"编译通过"代替。
